...
# ============================================================================
# ROUTES DU FORUM (STOCKAGE GITHUB)
# ============================================================================

# Modèles Pydantic pour le forum
...
class ForumStats(BaseModel):
    ...
    total_users: int = 0
    online_users: int = 0
    latest_topic: Optional[dict] = None

# ============================================================================
# GESTIONNAIRE DU FORUM (STOCKAGE GITHUB)
# ============================================================================
...
class ForumManager:
    ...
    def get_categories():
        ...
        data = GitHubManager.read_from_github('forum/categories.json', {'categories': []})
        return data.get('categories', [])
    
    @staticmethod
    def save_categories(categories):
        """Sauvegarde les catégories"""
        return GitHubManager.save_to_github('forum/categories.json', {'categories': categories}, "Update forum categories")
    
    @staticmethod
    def get_topics(category_id=None, page=1, per_page=20, sort='recent'):
        ...
        if category_id:
            topics = [t for t in topics if t.get('category_id') == category_id]
        
        # Trier
        if sort == 'recent':
            topics.sort(key=lambda x: x.get('last_reply_at', x.get('created_at')), reverse=True)
        elif sort == 'popular':
            topics.sort(key=lambda x: x.get('views', 0) + x.get('replies_count', 0)*2, reverse=True)
        elif sort == 'unanswered':
            topics = [t for t in topics if t.get('replies_count', 0) == 0]
    ...
    def get_topic(topic_id):
        ...
        for topic in data.get('topics', []):
            if topic.get('id') == topic_id:
                ...
                ForumManager.save_topics(data['topics'])
                return topic
        return None
    
    @staticmethod
    def save_topics(topics):
        """Sauvegarde tous les sujets"""
    ...
    def get_posts(topic_id, page=1, per_page=20):
        ...
        data = GitHubManager.read_from_github(f'forum/topics/{topic_id}/posts.json', {'posts': []})
    ...
    def create_post(post_data):
        ...
        path = f'forum/topics/{post_data["topic_id"]}/posts.json'
        data = GitHubManager.read_from_github(path, {'posts': []})
        ...
        if GitHubManager.save_to_github(path, data, f"New post in topic {post_data['topic_id']}"):
            # Mettre à jour le compteur de réponses du sujet
            ForumManager.update_topic_reply_count(post_data['topic_id'], post_data['author'])
            return new_post
        
        return None
    
    @staticmethod
    def update_topic_reply_count(topic_id, last_reply_by):
        """Met à jour le compteur de réponses d'un sujet"""
        data = GitHubManager.read_from_github('forum/topics.json', {'topics': []})
        
        for topic in data['topics']:
            if topic['id'] == topic_id:
                topic['replies_count'] = topic.get('replies_count', 0) + 1
                topic['last_reply_at'] = datetime.now().isoformat()
                topic['last_reply_by'] = last_reply_by
                break
    ...
    def update_category_stats(category_id):
        ...
        categories = ForumManager.get_categories()
        topics = GitHubManager.read_from_github('forum/topics.json', {'topics': []}).get('topics', [])
        
        for category in categories:
            if category['id'] == category_id:
                category_topics = [t for t in topics if t.get('category_id') == category_id]
                category['topics_count'] = len(category_topics)
                category['posts_count'] = sum(t.get('replies_count', 0) + 1 for t in category_topics)
                
                if category_topics:
                    latest = max(category_topics, key=lambda x: x.get('last_reply_at', x.get('created_at')))
                    category['last_post_at'] = latest.get('last_reply_at', latest.get('created_at'))
                    category['last_post_by'] = latest.get('last_reply_by', latest.get('author'))
                break
        
        ForumManager.save_categories(categories)
    
    @staticmethod
    def like_post(post_id, topic_id, username):
        """Like/unlike un post"""
        path = f'forum/topics/{topic_id}/posts.json'
        data = GitHubManager.read_from_github(path, {'posts': []})
        ...
        if GitHubManager.save_to_github(path, data, f"Post {action}"):
            return True
    ...
    def get_stats():
        ...
        topics = GitHubManager.read_from_github('forum/topics.json', {'topics': []}).get('topics', [])
        users = GitHubManager.read_from_github('database/users.json', {'users': []}).get('users', [])
        
        # Compter les posts
        total_posts = 0
        for topic in topics:
            posts_data = GitHubManager.read_from_github(f'forum/topics/{topic["id"]}/posts.json', {'posts': []})
            total_posts += len(posts_data.get('posts', []))
        
        # Trier les topics par date
        ...
        return {
            ...
            'online_users': 42,  # À implémenter avec Redis plus tard
            'latest_topic': {
                'id': latest_topic['id'],
                'title': latest_topic['title'],
                'author': latest_topic['author']
            } if latest_topic else None
        }
...
def api_forum_topics():
    ...
    result = ForumManager.get_topics(category, page, 20, sort)
    return jsonify(result)

@app.route('/api/forum/topics/<topic_id>', methods=['GET'])
def api_forum_topic(topic_id):
    """Récupère un sujet spécifique"""
    topic = ForumManager.get_topic(topic_id)
    if not topic:
        return jsonify({'error': 'Topic not found'}), 404
    return jsonify(topic)

@app.route('/api/forum/topics/<topic_id>/posts', methods=['GET'])
def api_forum_posts(topic_id):
    """Récupère les posts d'un sujet"""
    page = int(request.args.get('page', 1))
    result = ForumManager.get_posts(topic_id, page)
    return jsonify(result)

@app.route('/api/forum/topics', methods=['POST'])
@token_required
def api_forum_create_topic():
    ...
    try:
        ...
        topic_data = {
            ...
            'title': SecurityUtils.escape_text(data['title']),
            'content': data['content'],  # Markdown, pas besoin d'escape
            'author': g.user['username'],
            'author_id': g.user['id'],
            'tags': data.get('tags', []),
        ...
        topic = ForumManager.create_topic(topic_data)
...
@app.route('/api/forum/topics/<topic_id>/posts', methods=['POST'])
@token_required
def api_forum_create_post(topic_id):
    ...
    try:
        ...
        post = ForumManager.create_post(post_data)
...
@token_required
def api_forum_like_post(post_id):
    """Like/unlike un post"""
    try:
        data = request.get_json()
        topic_id = data.get('topic_id')
        
        if ForumManager.like_post(post_id, topic_id, g.user['username']):
            return jsonify({'success': True})
        else:
...
def forum_page():
    ...
    categories = ForumManager.get_categories()
    topics_result = ForumManager.get_topics(page=1, per_page=10)
    stats = ForumManager.get_stats()
    
    return render_template('forum.html',
                         categories=categories,
                         recent_topics=topics_result['topics'],
                         stats=stats,
                         user=session.get('user'))
...
def forum_category_page(category_id):
    ...
    topics_result = ForumManager.get_topics(category_id, page, 20, sort)
    
    return render_template('forum_category.html',
                         category=category,
                         topics=topics_result['topics'],
                         pagination=topics_result,
                         sort=sort,
                         user=session.get('user'))
...
def forum_topic_page(topic_id):
    ...
    page = int(request.args.get('page', 1))
    posts_result = ForumManager.get_posts(topic_id, page)
    
    return render_template('forum_topic.html',
                         topic=topic,
                         posts=posts_result['posts'],
                         pagination=posts_result,
                         user=session.get('user'))

@app.route('/forum/new')
@token_required
def forum_new_topic_page():
    """Page de création d'un nouveau sujet"""
    categories = ForumManager.get_categories()
...
def init_forum_categories():
    """Initialise les catégories du forum si elles n'existent pas"""
    categories = ForumManager.get_categories()
    ...
    if not categories:
        default_categories = [
            {
                ...
                'description': 'Talk about anything related to Zarch Hub',
                'icon': 'comments',
                'color': 'sky',
                'order': 1
            },
            {
                ...
                'description': 'Discuss package creation, PKGBUILD files, and build processes',
                'icon': 'cube',
                'color': 'purple',
                'order': 2
            },
            ...
            },
            {
                'id': 'announcements',
                'name': 'Announcements',
                'description': 'Official announcements about Zarch Hub updates and events',
                'icon': 'bullhorn',
                'color': 'yellow',
                'order': 4
            },
            {
                ...
                'description': 'Suggest new features and improvements for Zarch Hub',
                'icon': 'lightbulb',
                'color': 'pink',
                'order': 5
            },
            {
                'id': 'showcase',
                'name': 'Show & Tell',
                'description': 'Show off your packages and projects built with APKM',
                'icon': 'rocket',
                'color': 'indigo',
                'order': 6
            }
        ...
        ForumManager.save_categories(default_categories)
        app.logger.info("✅ Forum categories initialized")
